...
balance = action_helper.get_balance("H3")
cost = balance["cost"]
scale = balance["scale"]
...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        damage = 10 + unit_stats["power_current"] * action_helper.key_bonus_enemy(
        ...
        self_new_health = unit_stats["health_current"] + damage
        if(self_new_health > unit_stats["health_default"]):
            self_new_health = unit_stats["health_default"]
        sql = "UPDATE battle_unit_stats SET health_current = %s WHERE battle_id = %s AND team = %s AND slot = %s"
        params = (newhealth, battle_id, team, unit)
        cursor.execute(sql, params)
        ...
        summary_code = "t{}u{} h -{}".format(target_team, action_target_unit, damage)
        summary_text = "{} has stolen health from {} for {} health by using Ghosts & Stuff".format(
