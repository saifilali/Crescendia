...
balance = action_helper.get_balance("O8")
cost = balance["cost"]
scale = balance["scale"]
...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        damage = (15 + unit_stats["power_current"]) * action_helper.key_bonus_enemy(
        ...
        energy_sap = target_stats["energy_current"] / 2
        ...
        sql = "UPDATE battle_unit_stats SET health_current = %s, energy_current = energy_current/2 WHERE battle_id = %s AND team = %s AND slot = %s"
        ...
        summary_text = "{} has hit {} for {} damage with Thunderstruck, and sapped it of {} energy! ".format(
            unit_stats["title"], target_stats["title"], damage, energy_sap) + effective_text
