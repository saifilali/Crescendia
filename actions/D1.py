'''
Based off H0 (Hallellujah) without the healing component]
This should be fine
'''
import argparse
import pymysql
...
balance = action_helper.get_balance("D1")
cost = balance["cost"]
scale = balance["scale"]
...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        sql = "REPLACE INTO battle_effect_queue (battle_id, turn, team, unit, unit_key, unit_speed, action_code, action_target_team, action_target_unit, processed, turn_expire) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s, %s);"
        params = (battle_id, turn, unit_stats["team"], unit, unit_stats[
                  "song_key"], unit_stats["speed_current"], "H0E", "ally", target_stats["slot"], 0, str(int(turn) + 999))
        cursor.execute(sql, params)
        connection.commit()
        '''
        ...
        summary_text = "{} has protected {} from the next attack with  Somewhere over the rainbow!".format(
