...
balance = action_helper.get_balance("D4")
cost = balance["cost"]
scale = balance["scale"]
    ...
    if(unit_stats["health_current"] < 1):
        ...
        for target_stats in target_stats_all:
            ...
            params = (battle_id, turn, unit_stats["team"], unit, unit_stats[
                      "song_key"], unit_stats["speed_current"], "D4E", "ally", target_stats["slot"], 0, str(int(turn) + 1))
            cursor.execute(sql, params)
            summary_code += "t{}u{}".format(target_team, target_stats["slot"])
            connection.commit()
        ...
        summary_text = "{} casts Where are U now on all enemies, reducing their chance to hit".format(
            unit_stats["title"])
