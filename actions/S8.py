...
balance = action_helper.get_balance("S8")
cost = balance["cost"]
scale = balance["scale"]
...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        multiplier = 0.01 * (10 + unit_stats["power_current"] * action_helper.key_bonus_ally(
            action_helper.key_difference(target_stats["song_key"], unit_stats["song_key"])))
        if(target_stats["energy_current"] < (target_stats["energy_default"] * multiplier)):
            sql = "UPDATE battle_unit_stats SET energy_current = energy_default  WHERE battle_id = %s AND team = %s AND slot = %s"
