'''
modifed removal of debuffs to hopefully work.
'''
import argparse
import pymysql
import configparser
...
balance = action_helper.get_balance("D6")
cost = balance["cost"]
scale = balance["scale"]
...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        sql = "UPDATE battle_effect_queue SET processed = 1 WHERE battle_id = %s AND team = %s AND unit = %s AND type = 'debuff';"
        params = (battle_id, target_team, action_target_unit)
        cursor.execute(sql, params)
        ...
        summary_text = "{} has removed all debuffs from {} with Dirt off Yo Shoulder!".format(
