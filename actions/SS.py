...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        print(action_target_unit)
        cursor.execute(sql, params)
        target_stats = cursor.fetchone()
        energy_boost = unit_stats["power_current"] * action_helper.key_bonus_ally(
            action_helper.key_difference(target_stats["song_key"], unit_stats["song_key"]))
        new_energy = target_stats["energy_current"] + (energy_boost / 3)
        if(new_energy > target_stats["energy_default"]):
            new_energy = target_stats["energy_default"]
        if(target_stats["health_current"] > 0):
            ...
            params = (new_energy, battle_id, target_team, action_target_unit)
            cursor.execute(sql, params)

        '''
        This is the part where it updates some stuff in the receipt
        '''
        if(energy_boost > target_stats["energy_default"] * 0.2):
            effective_text = "It was extremely strong!"
            target_animation = "status_attack_effect_strong"
        elif(energy_boost > target_stats["energy_default"] * 0.1):
            effective_text = "It was strong!"
            target_animation = "status_attack_effect_moderate"
        elif(energy_boost > target_stats["energy_default"] * 0.05):
            effective_text = "It was weak!"
            target_animation = "status_attack_effect_weak"
        elif(energy_boost > 0):
            effective_text = "It did barely anything!"
            target_animation = "status_attack_effect_nothing"
        if(new_energy < 0):
            effective_text = target_stats["title"] + " died!"
            target_animation = target_animation + " status_attack_effect_nothing"
        sql = "UPDATE battle_unit_stats SET energy_current = %s WHERE battle_id = %s AND team = %s AND slot = %s"
        params = (new_energy, battle_id, target_team, action_target_unit)
        cursor.execute(sql, params)
        source_animation = "status_attack_cast"
        background_animation = ""
        summary_code = "t{}u{} -{}".format(target_team, action_target_unit, energy_boost)
        summary_text = "{} has given {} exactly {} units of energy! ".format(
            unit_stats["title"], target_stats["title"], energy_boost) + effective_text
