...
balance = action_helper.get_balance("O0")
cost = balance["cost"]
scale = balance["scale"]
    ...
    if(unit_stats["health_current"] < 1):
        ...
        for target_stats in target_stats_all:
            if(action_helper.is_immune(target_stats["immune"], battle_id, turn, target_stats["team"], target_stats["slot"]) == 0):
                damage = 10 + unit_stats["power_current"] * scale * action_helper.key_bonus_enemy(
                    action_helper.key_difference(target_stats["song_key"], unit_stats["song_key"]))
                if(target_stats["defense_current"] / 2 > damage):
                    damage = 1
                damage_string = damage_string + str(damage) + " "
                newhealth = target_stats["health_current"] - \
                    (damage - target_stats["defense_current"] / 2)
                if(newhealth < 0):
                    newhealth = 0
        ...
        summary_text = "{} casts Scary Monsters & Nice Sprites, damaging every enemy by {}".format(
