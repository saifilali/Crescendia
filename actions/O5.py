...
balance = action_helper.get_balance("O5")
cost = balance["cost"]
scale = balance["scale"]
...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        total_missing_health = 0
        for target_stats in target_stats_all:
            total_missing_health = total_missing_health + \
                (target_stats["health_default"] - target_stats["health_current"])
        for target_stats in target_stats_all:
            if(action_helper.is_immune(target_stats["immune"], battle_id, turn, target_stats["team"], target_stats["slot"]) == 0):
                damage = (50 + unit_stats["power_current"]) * 0.01 * action_helper.key_bonus_enemy(
                    action_helper.key_difference(target_stats["song_key"], unit_stats["song_key"])) * (total_missing_health / 4)
        ...
        summary_text = "{} has cased Symphony of Destruction, causing {} damage".format(
