    ...
    if(unit_stats["health_current"] < 1):
        ...
        for target_stats in target_stats_all:
            ...
            if(target_stats["health_current"] > 0):
                ...
                params = (new_energy, battle_id, target_team, target_stats["slot"])
                cursor.execute(sql, params)

        source_animation = "status_attack_cast"
        background_animation = "status_attack_bg_animation"
        target_animation = "status_attack_effect"
        summary_code = "format this later"
        summary_text = "{} has given energy to every alive ally unit".format(
            unit_stats["title"])
