'''
The deflect component MIGHT work
'''
import argparse
import pymysql
import configparser
...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        params = (battle_id, turn, unit_stats["team"], unit, unit_stats[
                  "song_key"], unit_stats["speed_current"], "D8E", "ally", target_stats["slot"], 0, str(int(turn) + 999))
        cursor.execute(sql, params)
        connection.commit()
        '''
        ...
        summary_text = "{} has protected {} from the death with I Will Survive!".format(
