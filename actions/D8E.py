...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        if(target_stats["health_current"] < 1):
            sql = "UPDATE battle_unit_stats SET health_current = 1 WHERE health_current < 1 AND battle_id = %s AND team = %s AND slot = %s"
            params = (battle_id, target_team, action_target_unit)
            cursor.execute(sql, params)
            ...
            summary_text = "{} has kept {} alive with I Will Survive!".format(
