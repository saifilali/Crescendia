'''
This heal action is probably buggy. It should only cast the effect, and the effect needs to trigger when the target is dead
'''
import argparse
import pymysql
import configparser
...
with connection.cursor() as cursor:
    ...
    if(unit_stats["health_current"] < 1):
        ...
        sql = "REPLACE INTO battle_effect_queue (battle_id, turn, team, unit, unit_key, unit_speed, action_code, action_target_team, action_target_unit, processed, turn_expire, type) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s, %s, 'buff');"
        params = (battle_id, turn, unit_stats["team"], unit, unit_stats[
                  "song_key"], unit_stats["speed_current"], "H4E", "ally", target_stats["slot"], 0, str(int(turn) + 999))
        cursor.execute(sql, params)
        connection.commit()
        ...
        summary_text = "{} has blessed {} with Never Gonna Give You Up!".format(
